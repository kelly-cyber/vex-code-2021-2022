...
void moveFwdRev2(double motorPower, int Degrees, double myHeading){ //neg degrees to move rev

  LeftDrive.setPosition(0, degrees); //set y encoder to 0
  double KP = 1;
  motorPower = motorPower*0.01; //pct

  //set velocities for drivetrain motors based on heading
  double headingErr = InternalSensor.heading() - myHeading; //pos means right needs to travel more
  double rCorrect = 0;
  double lCorrect = 0;
  ...
  while(Error != 0){
    ...
    if(Error > 8){
      RightDriveMotors.setVelocity(motorPower*powerDif + 5 + rCorrect, pct);
      RightDrive.setVelocity(motorPower*powerDif + 5 + rCorrect, pct);
      LeftDriveMotors.setVelocity(motorPower*powerDif + 5 + lCorrect, pct);
      LeftDrive.setVelocity(motorPower*powerDif + 5 + lCorrect, pct);
  ...
  if(Error == 0 && headingErr != 0){
    TurnInPlace(headingErr, 7, 1);
  }
...
void TurnInPlace(float Deg, double slowDown, int minSpeed){ 
  ...
  // if(inertDif*inertDif <= 0.5){
  //   wait(10, msec);
  // }
  while (inertDif*inertDif > 0.3){ // error tolerance
